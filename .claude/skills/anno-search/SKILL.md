...
- **The API is undocumented.** `/anno-suche/rest` is the private backend of ANNO's search app. If it is ever redeployed at a different path the server answers HTTP 200 with the app's HTML shell rather than a 404; the client detects that and says so explicitly rather than failing obscurely. If you see that error, the endpoint has moved and the client needs updating — it is not a transient fault to retry.

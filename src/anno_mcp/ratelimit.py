...
The ÖNB publishes no rate limit for ANNO, and none was observed while the client
was being written: roughly fifty requests spaced three seconds apart drew no 429,
no challenge page and no visible throttling. That is an absence of evidence
rather than a documented allowance, so the default here matches the Gallica
client's three seconds. ANNO's search endpoint is an undocumented internal API
serving a public reading room; the asymmetry is the usual one, where pacing
conservatively costs seconds and being wrong costs the session.
...
MIN_INTERVAL_ENV_VAR = "ANNO_MIN_REQUEST_INTERVAL"

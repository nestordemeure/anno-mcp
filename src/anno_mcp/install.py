...
def install_claude(enable_advanced_search=False):
    ...
    try:
        cmd_args = [
            ...
            "anno",
            "--",
            "uv", "--directory", str(project_dir), "run", "anno-mcp"
        ]
...
def main():
    """Install anno MCP server to all detected CLIs"""
    parser = argparse.ArgumentParser(description='Install ANNO MCP Server')
    ...
    print(f"Installing anno MCP server...")
    ...
    if not args.enable_advanced_search:
        print(f"\n  To enable advanced search: run 'uv run anno-mcp-install --enable-advanced-search'")
